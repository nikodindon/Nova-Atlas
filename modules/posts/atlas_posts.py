...
class PostsGenerator:
    ...
    def _generate_posts(self, articles: list, now: datetime) -> list:
        ...
        # FIX : les dicts ne sont pas hashables, on ne peut pas les mettre dans un set.
        # On utilise les id() Python (adresses mémoire) pour identifier les articles chauds.
        hot_articles_list = [
            art for _, arts in topics for art in arts if len(arts) >= 2
        ]
        hot_ids = {id(a) for a in hot_articles_list}

        for a in articles:
            if id(a) not in hot_ids:
